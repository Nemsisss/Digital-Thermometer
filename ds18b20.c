...
void ds_write1bit(void)
{
    ...
    // and with a zero in the PORT bit it will go low.
    // DDR bit = 1, PORT bit = 0
    DDRC |= (1<<PC3); //set DDRC as output
    _delay_us(2); //hold the bus at low for 2us

    DDRC &= ~(1<<PC3); //release the bus to let it go to high again
    _delay_us(60); //to complete the 60us WRITE "1" SLOT
}
...
void ds_write0bit(void)
{
    ...
    DDRC |= (1<<PC3); //set DDRC as output
    _delay_us(60); //hold the bus at low for 60us
    DDRC &= ~(1<<PC3); //release the bus to let it go to high again
    _delay_us(2); //delay 2us as recovery time between individual write slots
...
unsigned char ds_readbit(void)
{
    ...
    unsigned char ds18b20_output;
    DDRC |= (1<<PC3); //set PORTC as output
    _delay_us(2); //hold the bus at low for 2us
    DDRC &= ~(1<<PC3); //set PORTC pin3 as input to read ds18b20's output
    _delay_us(10);
    if(PINC & (1<<3))
    {
        ds18b20_output=1;
    ...
    _delay_us(50); //to complete the 62us read time slot
    return ds18b20_output;
}
